...
class Post {
    ...
    static async create(postData) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let newPostData = await db.query(`INSERT INTO posts (title, author, text) VALUES ($1, $2, $3) RETURNING *;`, [postData.title, postData.author, postData.text]);
    ...
    //     return new Promise(async (resolve, reject) => {
    //         try {
    //             // Changed "DESTROY" for delete in query for result
    //             const result = await db.query('DELETE FROM posts WHERE id = $1 RETURNING author_id', [this.id]);
